...
const RevisionNotasContainer = (props) => {
  ...
  const generar_tabla = () => {
    let columns = [
      ...
      {
        cell: (row) => (
          <button className="btn btn-primary" id={row.id} onClick={() => props.revisionTestProfesor(params.id, row.id)}>
            Revisar
          </button>
        ),
        ignoreRowClick: true,
    ...
    return (
      <div className="index-body">
        <div className="card tabla-notas">
          <DataTable pointerOnHover theme={"default"} title={title} columns={columns} expandableRows expandableRowsComponent={ExpandedComponent} data={data} pagination paginationComponentOptions={paginationComponentOptions} />
        </div>
      </div>
    );
  } else {
